...
from common import ensure_dev_project
...
def memory_tracker(track_memory: bool = False):
    ...
    try:
        ...
        if track_memory and MEMORY_TRACKING_AVAILABLE:
            ...
            logger.warning("Memory tracking requested but psutil/tracemalloc not available. Install with: pip install psutil")
...
def mock_firestore_operations():
    ...
    mock_db.write_dogs = lambda dogs, dry_run=False: {"dogs_written": len(dogs) if not dry_run else 0, "batch_count": 1}
    mock_db.purge_stale_dogs = lambda ids, dry_run=False: 0
    mock_db.get_db = lambda: None
    sys.modules['db'] = mock_db

def run_pipeline_performance_test(dog_limit: int, dry_run: bool = True, track_memory: bool = False, memos_mode: str = "none", max_concurrent_scrapes: int = 1) -> PerformanceMetrics:
    """
    ...
    """
    print(f"DEBUG: run_pipeline_performance_test called with dog_limit={dog_limit}, dry_run={dry_run}")
    import pipeline
    ...
    with memory_tracker(track_memory) as memory_info:
        ...
        with unittest.mock.patch('api_client.get_animal_events') as mock_events, \
             unittest.mock.patch('api_client.get_people') as mock_people:
            ...
            stats = pipeline.run_etl_process(dry_run=dry_run, animal_limit=dog_limit, timing_hooks=timing_hooks, memos_mode=memos_mode, max_concurrent_scrapes=max_concurrent_scrapes)

    # Calculate stage durations
    extract_events_time = (stage_timings["extract_events_end"] - stage_timings["extract_events_start"]) if stage_timings["extract_events_end"] and stage_timings["extract_events_start"] else 0
    extract_people_time = (stage_timings["extract_people_end"] - stage_timings["extract_people_start"]) if stage_timings["extract_people_end"] and stage_timings["extract_people_start"] else 0
    transform_mappings_time = (stage_timings["transform_mappings_end"] - stage_timings["transform_mappings_start"]) if stage_timings["transform_mappings_end"] and stage_timings["transform_mappings_start"] else 0
    scrape_transform_time = (stage_timings["scrape_transform_end"] - stage_timings["scrape_transform_start"]) if stage_timings["scrape_transform_end"] and stage_timings["scrape_transform_start"] else 0
    load_time = (stage_timings["load_end"] - stage_timings["load_start"]) if stage_timings["load_end"] and stage_timings["load_start"] else 0
...
def print_performance_report(metrics_list: List[PerformanceMetrics], dog_limits: List[int], iterations: int):
    ...
    for i, dog_limit in enumerate(dog_limits):
        ...
        limit_metrics = [m for m in metrics_list if abs(m.dogs_processed - dog_limit) <= 2 or m.dogs_processed <= dog_limit]
        ...
        avg_transform_mappings = sum(m.transform_mappings_time for m in limit_metrics) / len(limit_metrics)
        avg_scrape_transform = sum(m.scrape_transform_time for m in limit_metrics) / len(limit_metrics)
        ...
        print(f"\nData Processing:")
        print(f"Average dogs processed: {sum(m.dogs_processed for m in limit_metrics) / len(limit_metrics):.1f}")
        print(f"Average dogs written: {sum(m.dogs_written for m in limit_metrics) / len(limit_metrics):.1f}")
        print(f"Average dogs deleted: {sum(m.dogs_deleted for m in limit_metrics) / len(limit_metrics):.1f}")
...
def main():
    ...
    parser.add_argument("--dogs", type=int, default=5,
                       help="Number of dogs to test with (default: 5)")
    ...
    if args.verbose:
        ...
        print(f"DEBUG: SHELTERLUV_API_KEY = {'SET' if os.environ.get('SHELTERLUV_API_KEY') else 'NOT SET'}")
