...
from config import EtlConfig, assert_dev_safe_config
...
Examples:
  ...
  python run_etl_local.py --limit 10 --dry-run          # Preview with 10 animals
    ...
    parser.add_argument(
        '--dry-run',
        action='store_true',
        help='Run in dry-run mode (no writes to Firestore)'
    )
    parser.add_argument(
        '--limit',
        type=int,
        metavar='N',
        help='Limit processing to first N animals (for testing)'
    )
    parser.add_argument(
        '--memos-mode',
        choices=['none', 'api'],
    ...
    logger.info(f"Config: profile={config.env_profile.value}, project={config.project_id}, memos={config.memos_mode}")
    logger.info("=" * 80)
