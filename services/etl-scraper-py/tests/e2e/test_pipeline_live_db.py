...
from db import get_db, get_dog_collection, clear_all_dogs, write_dogs, purge_stale_dogs
...
if missing_creds:
    pytest.skip(f"Missing required ShelterLuv credentials: {', '.join(missing_creds)}", allow_module_level=True)
...
def validate_test_environment():
    ...
    assert "prod" not in project_lower, f"Refusing to run E2E tests against production project: {project_id}"
    assert "live" not in project_lower, f"Refusing to run E2E tests against live project: {project_id}"
...
class TestLiveDBPipeline:
    ...
    def _create_e2e_config(self, test_collection, dry_run=False, animal_limit=None):
        ...
        return EtlConfig(
            ...
            secrets=SecretsConfig(mode=SecretsMode.ENV, project_id=os.environ.get("GCP_PROJECT") or os.environ.get("GOOGLE_CLOUD_PROJECT")),
            memos_mode="api",
    ...
    def test_etl_writes_valid_dog_documents(self, test_collection):
        ...
        if len(docs) > 0:
            ...
            for snap in docs[:sample_size]:
                ...
                doc_id = snap.id
                assert doc_id == str(data["Internal-ID"]), f"Document ID {doc_id} should match Internal-ID {data['Internal-ID']}"
    ...
    def test_etl_idempotency_behavior(self, test_collection):
        ...
        assert stats2["dogs_written"] <= stats1["dogs_written"], \
    ...
    def test_etl_schema_consistency(self, test_collection):
        ...
        if stats["dogs_written"] == 0:
            pytest.skip("No dogs written - skipping schema validation (may be no ShelterLuv data available)")
        ...
        for snap in docs:
            ...
            for field in boolean_fields:
                if field in data and not isinstance(data[field], bool):
                    schema_violations.append(f"Doc {snap.id}: {field} should be bool, got {type(data[field])}")
    ...
    def test_purge_stale_dogs_with_controlled_data(self, test_collection):
        ...
        assert remaining_ids == active_ids, f"Remaining dogs should be {active_ids}, got {remaining_ids}"
