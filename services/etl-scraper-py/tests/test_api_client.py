...
from api_client_animals import get_all_animals_in_custody, get_animal_by_internal_id, get_animals_by_ids, MAX_ANIMALS
from api_client_events import get_animal_events, MAX_EVENTS_PER_ANIMAL
from api_client_people import get_people, MAX_PEOPLE_PER_ANIMAL
from api_client_memos import get_animal_memos, get_animals_memos_batch, MemoResult
from api.api_client_base import MAX_RETRIES, REQUEST_TIMEOUT
...
class TestGetAllAnimalsInCustody:
    ...
    @patch('api_client_base.requests.get')
    ...
    def test_get_all_animals_multiple_pages(self, mock_get):
        ...
        second_response.json.return_value = {
            "animals": [
                {"Internal-ID": "101", "Name": "Dog101", "Status": "Foster Available"}
            ]
        }
    ...
    def test_get_all_animals_filters_custody_status(self, mock_get):
        ...
        mock_response.json.return_value = {
            "animals": [
                ...
                {"Internal-ID": "3", "Name": "PendingDog", "Status": "Headquarters Unavailable - Pending Medical Exam"},
    ...
    def test_get_all_animals_max_limit_enforced(self, mock_get):
        ...
        mock_response = Mock()
        # Create more animals than MAX_ANIMALS
        animals = [{"Internal-ID": str(i), "Name": f"Dog{i}", "Status": "Headquarters Available"}
    ...
    def test_get_animal_events_filters_relevant_types(self, mock_get):
        ...
        mock_response.json.return_value = {
            "events": [
                ...
                {"Type": "Irrelevant", "Date": "2023-01-06", "AnimalID": "1"},  # Should be filtered out
            ]
        }
        mock_response.raise_for_status.return_value = None
        mock_get.return_value = mock_response
        ...
        assert len(result) == 5  # Foster, FosterReturn, Event, Adoption, Intake (Irrelevant filtered out)
...
class TestGetAnimalMemos:
    ...
    def test_get_animal_memos_success_with_documents(self, mock_get):
        ...
        expected = MemoResult(internal_id="123", memos_html="Medical note: healthy dog<br>Behavior note: friendly", source="api", api_failed=False)
    ...
    def test_get_animal_memos_success_with_memos_field(self, mock_get):
        ...
        expected = MemoResult(internal_id="123", memos_html="Intake note: found as stray<br>Medical note: vaccinated", source="api", api_failed=False)
        assert result == expected
    ...
    def test_get_animal_memos_success_with_string_memos(self, mock_get):
        ...
        expected = MemoResult(internal_id="123", memos_html="Single memo content", source="api", api_failed=False)
        assert result == expected
...
class TestGetAnimalsMemosBatch:
    ...
    def test_get_animals_memos_batch_concurrent_success(self, mock_individual):
        ...
        mock_individual.side_effect = [
            MemoResult(internal_id="123", memos_html="Memo for 123", source="api", api_failed=False),
            MemoResult(internal_id="456", memos_html="Memo for 456", source="api", api_failed=False),
            MemoResult(internal_id="789", memos_html="", source="api", api_failed=False)
        ]
        ...
        expected = {
            "123": MemoResult(internal_id="123", memos_html="Memo for 123", source="api", api_failed=False),
            "456": MemoResult(internal_id="456", memos_html="Memo for 456", source="api", api_failed=False),
            "789": MemoResult(internal_id="789", memos_html="", source="api", api_failed=False)
        }
        assert result == expected
