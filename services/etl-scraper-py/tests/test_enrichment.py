...
class TestStructuredNotesParsing:
    ...
    @pytest.fixture
    def parser(self):
        """Create a parser instance for testing (no browser needed for HTML parsing)."""
        return ShelterLuvParsers(None, None)

    def test_empty_memos(self, parser):
        """Test handling of empty memos."""
        result = parser.parse_memos_by_type("")
        assert result["PersonalityNotes"] == ""
        assert result["IntakeNotes"] == ""
        assert result["MedicalNotes"] == ""

    def test_personality_keywords(self, parser):
        ...
        """
        result = parser.parse_memos_by_type(memos)
    ...
    def test_medical_keywords(self, parser):
    ...
    def test_intake_keywords(self, parser):
    ...
    def test_mixed_categories(self, parser):
        ...
        assert "intake" in result["IntakeNotes"].lower() or "rescued" in result["IntakeNotes"].lower()
        assert "medical" in result["MedicalNotes"].lower() or "treatment" in result["MedicalNotes"].lower()
        assert "friendly" in result["PersonalityNotes"].lower() or "temperament" in result["PersonalityNotes"].lower()

    def test_html_tag_stripping(self, parser):
    ...
    def test_priority_order_medical_over_intake(self, parser):
    ...
    def test_short_sections_ignored(self, parser):
