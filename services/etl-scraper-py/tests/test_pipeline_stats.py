...
from pipeline import compute_stats, run_etl_process, PipelineStats, ExtractResult, TransformResult, LoadResult
from transform import transform, TransformConfig
from enrichment import build_dog_record
from errors import EtlError, ApiError
...
class TestTransformInvalidCount:
    ...
    @patch('transform._scrape_animals_concurrent')
    @patch('foster_mapping.build_foster_maps')
    @patch('foster_mapping.build_event_maps')
    @patch('api_client_memos.get_animals_memos_batch')
    def test_transform_all_valid(self, mock_get_memos, mock_build_events, mock_build_foster, mock_scrape):
        """Test transform with all valid records."""
        # Setup extract result
        extract_result = ExtractResult(
            in_custody_ids={"1"},
            animals_by_id={
                "1": {"Internal-ID": "1", "ID": "A1", "Name": "Dog 1", "Status": "AVAILABLE", "Age": "3 years"}
            },
            events=[],
            people=[],
            existing_metadata={}
        ...
        mock_get_memos.return_value = {}

        with patch('enrichment.build_dog_record') as mock_build_dog:
            mock_build_dog.return_value = {"Internal-ID": "1", "Name": "Dog 1"}

            result = transform(extract_result, {}, TransformConfig(memos_mode="none", max_concurrent_scrapes=1))
    ...
    @patch('foster_mapping.build_event_maps')
    @patch('api_client_memos.get_animals_memos_batch')
    def test_transform_with_invalid(self, mock_get_memos, mock_build_events, mock_build_foster, mock_scrape):
    ...
    @patch('foster_mapping.build_event_maps')
    @patch('api_client_memos.get_animals_memos_batch')
    def test_transform_all_invalid(self, mock_get_memos, mock_build_events, mock_build_foster, mock_scrape):
        """Test transform with all invalid records (missing required fields)."""
        # Setup extract result
        extract_result = ExtractResult(
            in_custody_ids={"1"},
            animals_by_id={
                "1": {"Internal-ID": "1", "Name": "Dog 1"}  # Missing required ID field
            },
            events=[],
            people=[],
            existing_metadata={}
...
class TestPipelineFailureHandling:
    ...
    def test_events_fetch_failure_tracked(self):
        ...
        extract_result = ExtractResult(
            in_custody_ids={"1"},
            animals_by_id={"1": {"Internal-ID": "1", "ID": "A1", "Name": "Test Dog", "Status": "AVAILABLE", "Age": "3 years"}},
