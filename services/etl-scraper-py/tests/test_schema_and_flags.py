...
- .env.local file in services/etl-scraper-py/ (for local development)
...
1. Create services/etl-scraper-py/.env.local
...
from schema import (
    _normalize_status, _normalize_age_years, _build_age_display,
    validate_dog_record, get_required_fields, DOG_SCHEMA
)
...
class TestETLRequiredFieldsEmission:
    ...
    def test_etl_emits_all_required_fields(self):
        ...
        with mock.patch('transform._scrape_animals_concurrent', return_value={"123": {}}), \
             mock.patch('transform._fetch_memos_if_needed', return_value={}):
