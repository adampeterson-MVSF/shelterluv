...
from ..schema import _normalize_status, get_normalized_statuses, DOG_SCHEMA
...
def analyze_statuses_from_user_input():
    ...
    print()
    print(f"Summary: {included_count} statuses would be INCLUDED, {excluded_count} would be EXCLUDED")
    print()
...
def analyze_api_statuses():
    ...
    try:
        ...
        print(f"By current logic: {len(included_animals)} would be INCLUDED, {len(excluded_animals)} would be EXCLUDED")
        print()
        ...
        for i, animal in enumerate(all_animals[:10]):
            ...
            print("2d: ID={} Name='{}' Status='{}' (normalized={}) CurrentLocation={}".format(
