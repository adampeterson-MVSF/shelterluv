...
from .api_client_base import BASE_URL, _make_api_request, _validate_animal_records
...
def get_animals_by_ids(
    internal_ids: Collection[str], api_key: str, max_workers: int = 5
    ...
    def fetch_animal(internal_id: str) -> tuple[str, Dict[str, Any]]:
        try:
            ...
            logger.warning(
                f"No animal data in API response for {internal_id}, response keys: {list(data.keys())}"
            )
            return internal_id, {}
...
def find_shelterluv_ids_by_muttville_ids(
    muttville_ids: Collection[str], api_key: str, max_pages: int = 10
) -> Dict[str, str]:
    """
    ...
    for page in range(1, max_pages + 1):
        params = {"page": page, "per_page": 100, "include": "Internal-ID,ID"}
