    ...
    if emulator_host:
        ...
        try:
            ...
            if os.path.exists(service_account_path):
                ...
                firebase_admin.initialize_app(cred, {"projectId": "muttville"})
                ...
                firebase_admin.initialize_app()
                logger.info(
                    "Initialized Firebase app with default credentials (service account key not found)"
                )
            return firestore.client()
...
def write_dogs(dogs_data: List[Dict[str, Any]], dry_run: bool = False) -> Dict[str, int]:
    ...
    if dry_run:
        ...
        return {
            "dogs_written": len(dogs_data),
            "batch_count": (len(dogs_data) + BATCH_SIZE - 1) // BATCH_SIZE,
        }
    ...
    for chunk in _chunk_operations(dogs_data, BATCH_SIZE):
        ...
        if chunk_written > 0:
            try:
                logger.info(
                    f"Committing batch with {chunk_written} dogs to collection {get_dog_collection()}"
                )
                result = batch.commit()
                ...
                batch_count += 1
                logger.info(
                    f"Successfully committed batch {batch_count}, total written so far: {written_count}"
                )
        ...
        try:
            ...
            if doc.exists:
                ...
                metadata[str(internal_id)] = {
                    ...
                    "etl_last_processed_at": data.get("metadata", {}).get(
                        "etl_last_processed_at", ""
                    ),
                }
        except Exception as e:
...
def _add_etl_metadata(dog: Dict[str, Any], source_updated_at: str = None) -> Dict[str, Any]:
    ...
    # Always update the ETL processing timestamp (RFC 3339 UTC with Z)
    metadata["etl_last_processed_at"] = datetime.now(UTC).isoformat().replace("+00:00", "Z")
...
def write_dogs_and_purge_stale(
    dogs_data: List[Dict[str, Any]], active_dog_ids: Set[str], dry_run: bool = False
) -> Dict[str, int]:
    """
    Single function to write dogs and purge stale records.
    Returns stats dict with dogs_written, dogs_deleted.
    """
    # Write dogs first
    write_stats = write_dogs(dogs_data, dry_run=dry_run)

    # Then purge stale records
    dogs_deleted = purge_stale_dogs(active_dog_ids, dry_run=dry_run)

    return {"dogs_written": write_stats["dogs_written"], "dogs_deleted": dogs_deleted}
