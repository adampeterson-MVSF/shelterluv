...
def compute_derived_flags(
    dog: Dict[str, Any], config: FlagsConfig = DEFAULT_FLAGS_CONFIG
) -> Dict[str, Any]:
...
def compute_is_in_custody(
    status: str, dog_data: Dict[str, Any], config: FlagsConfig = DEFAULT_FLAGS_CONFIG
) -> bool:
    """
    Compute IsInCustody.

    Single source of truth for custody logic:
    - All animals in the ETL pipeline come from ShelterLuv's "In Custody" view
    - Terminal statuses (e.g., ADOPTED) indicate the dog is no longer in custody
    - All other animals from the "In Custody" view are in custody

    This simplified rule trusts the ShelterLuv "In Custody" view entirely.
    We do not mix this with custom rules or API flags to avoid confusion.

    Args:
        status: Normalized status (AVAILABLE, ADOPTED, etc.)
        dog_data: Dog data dict (unused but kept for API compatibility)
...
def compute_is_hospice(dog: Dict[str, Any], config: FlagsConfig = DEFAULT_FLAGS_CONFIG) -> bool:
    ...
    if isinstance(fields_to_check[3], list):
        fields_to_check[3] = " ".join(str(attr) for attr in fields_to_check[3])
...
def compute_is_event_dog(dog: Dict[str, Any], config: FlagsConfig = DEFAULT_FLAGS_CONFIG) -> bool:
    ...
    if isinstance(fields_to_check[2], list):
        fields_to_check[2] = " ".join(str(attr) for attr in fields_to_check[2])
