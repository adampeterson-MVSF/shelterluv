...
class ShelterLuvSession:
    ...
    def _fill_input_by_label_regex(
        self, regex: re.Pattern, value: str, timeout_ms: int = 5000
    ) -> None:
    ...
    def __enter__(self):
        ...
        self.context = self.browser.new_context(user_agent="Muttville-Internal-Scraper-Bot")
    ...
    def _login(self):
        ...
        try:
            self.page.goto(
                "https://new.shelterluv.com/login", wait_until="domcontentloaded", timeout=30000
            )

            # Try primary CSS inputs first
            try:
                self.page.locator(SELECTORS["login_username"]).first.fill(
                    self.username, timeout=3000
                )
                self.page.locator(SELECTORS["login_password"]).first.fill(
                    self.password, timeout=3000
                )
            except Exception:
                # Fallback to accessible-name labels if CSS not found
                self._fill_input_by_label_regex(
                    SELECTORS["login_username_label_re"], self.username, timeout_ms=4000
                )
                self._fill_input_by_label_regex(
                    SELECTORS["login_password_label_re"], self.password, timeout_ms=4000
                )
            ...
            try:
                self.page.wait_for_url(SELECTORS["post_login_url_pattern"], timeout=30000)
            except Exception:
                ...
                if "new.shelterluv.com" not in self.page.url:
                    raise ScraperError(
                        f"Login failed: not on expected ShelterLuv domain. Current URL: {self.page.url}"
                    )
        ...
        try:
            ...
            if "tab=animals" in current_url and "section=" in current_url:
                try:
                    ...
                    self.page.goto(
                        "https://new.shelterluv.com/dashboard?tab=animals&section=in_custody"
                    )
                    self.page.wait_for_timeout(2000)
                except Exception:
                    pass
            ...
            if not table_found:
                ...
                raise ScraperError(
                    "Could not find table elements. The In Custody view may load data via API endpoints rather than direct HTML. Consider using the Network tab to find the JSON endpoint that populates this table."
                )
