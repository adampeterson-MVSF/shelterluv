...
class TestWriteDogs:
    ...
    @patch("db.UTC")
    @patch("db.get_db")
    def test_write_dogs_uses_correct_document_id(self, mock_get_db, mock_utc, mock_datetime):
        """Test write_dogs uses Internal-ID as document ID."""
        # Mock datetime.now(UTC) to return a consistent timestamp with +00:00 so replace works
        ...
        expected_dog = {
            "Internal-ID": "123",
            "Name": "Dog123",
            "metadata": {"etl_last_processed_at": "2023-01-01T12:00:00Z"},
        }
